...
class SudokuValidator(object):
    ...
    def checkrowcol(self, row, col):
        ...
        currentBox = []
        for i in range(BoxStartEndRange[str(correctk)][0][0],
                       BoxStartEndRange[str(correctk)][0][1]):
            for j in range(BoxStartEndRange[str(correctk)][1][0],
                           BoxStartEndRange[str(correctk)][1][1]):
            ...
            for i in range(BoxStartEndRange[str(k)][0][0],
                ...
                for j in range(BoxStartEndRange[str(k)][1][0],
                    ...
                    if self.grid[i][j] != 0 and (
                            self.grid[i][j] not in currentBox):
                        currentBox.append(self.grid[i][j])
                    elif self.grid != 0:
                        return 1
            for i in range(BoxStartEndRange[str(k)][0][0],
    ...
    def validateBoard(self, solutionFound=False):
        ...
        for row in range(0, numberOfRows):
            currentRow = []
            for cell in self.grid[row]:
                if solutionFound and (cell not in currentRow) and (cell > 0 and
                                                                   cell < 9):
                    currentRow.append(cell)
                    continue
        ...
        for col in range(0, numberOfColumns):
            currentColumn = []
            for cell in transposeMatrix[col]:
                if solutionFound and (cell not in currentColumn) and (
                        cell > 0 and cell < 9):
                    currentColumn.append(cell)
                    continue
                elif not solutionFound and (cell < 0 or cell > 9):
        ...
        for k in range(0, 9):
            ...
            for i in range(BoxStartEndRange[str(k)][0][0],
                ...
                for j in range(BoxStartEndRange[str(k)][1][0],
                               BoxStartEndRange[str(k)][1][1]):
                    if solutionFound and (
                            self.grid[i][j] > 0 and self.grid[i][j] < 9) and (
                                self.grid[i][j] not in currentBox):
                        currentBox.append(self.grid[i][j])
                        continue
                    elif not solutionFound and (cell < 0 or cell > 9):
